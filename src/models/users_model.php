...
class Users_Model extends Model
{
  ...
  public function addUser($login, $name, $pass, $email, $userRole, $description)
  {
    ...
    $userExists  = $this->_db->select(
      'SELECT Count(*) as total  FROM Users  WHERE lower(login) = :login',
    ...
    return  $this->_db->insert('Users', array(
      "id" => Uuid::guidv4(), "login" => substr(trim($login), 0, 29),
  ...
  public function deleteUser($id, $currentUserId)
  {
    if (!empty($id) && $id !== $currentUserId) {
      return $this->_db->delete('Users', array("id" => $id), 1);
    }
